...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // 获取当前文章信息
      articleModification({ id: this.id }).then((res) => {
        console.log(res.data);
        // form.synopsis
        this.form.title = res.data.title;
        this.form.label = res.data.label;
        this.form.synopsis = res.data.synopsis;
        this.md_path = res.data.md_path;
        // this.synopsis = "http://localhost:2000" + res.data.tp_path;
        // console.log("http://localhost:2000" + res.data.tp_path);
      });
      text_get_md({ id: this.id }).then((res) => {
        this.value = res.data.md_path;
      });
      // 获取当前文章内容（md文章）
    },
    go_sy() {
    ...
    onSubmit() {
      ...
      if (
        ...
        this.$confirm(`${vlau || title || label || synopsis}不能为空！！！`, {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        });
